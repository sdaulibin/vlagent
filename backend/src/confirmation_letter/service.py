...
9. **start_date (起始日期)**
   - 【最重要】必须从包含「注销的银行存款账户」的表格上方的文字描述中提取，如「3. 自xxxx年x月x日起至...」中的「xxxx年x月x日」
   - 【禁止】绝对不能从正文第一段「正在对本公司xxxx到xxxx的财务报表进行审计」中提取日期！
10. **end_date (终止日期)**
   - 【最重要】必须从包含「注销的银行存款账户」的表格上方的文字描述中提取，如「至xxxx年x月x日期间内注销...」中的「xxxx年x月x日」
   - 【禁止】绝对不能从正文第一段的审计期间描述中提取！
11. **seal_date (印章日期)**
   - 提取被询证单位（客户公司）落款处的日期
...
def _extract_date_range(text: str) -> tuple[str, str]:
    ...
    # 仅保留具有高置信度的账户表格特征句式
    # 彻底杜绝回退到 "纯日期连读（如 2024年1月-2024年12月）" 以免匹配到正文抬头的审计期间
    patterns = [
        # 最完整的特征：带有“注销”或“期间”，可以完全忽略“自”、“至”被 OCR 识别成什么
        # 匹配如：3. 自[2024年1月1日]起至[2024年12月31日]期间内注销的银行存款账户
        rf"({DATE_PAT})[^\d]{{1,20}}({DATE_PAT})[^\d]{{1,20}}(?:期间|注销|账户)",
        # 常规特征：明确带「自/白/目/从」和「至/到」，且中间夹着日期。
        # 严卡字符距离，且中间必须有“至/到”，避免匹配到抬头描述的 "-"
        rf"[自白目从][^\d]{{0,15}}({DATE_PAT})[^\d]{{1,15}}(?:至|到)[^\d]{{0,15}}({DATE_PAT})",
    ]

    for pattern in patterns:
        matches = re.findall(pattern, raw)
        if matches:
            # 明确带有表格特征的格式非常准，通常直接取第一个即可
            return matches[0]

    # 如果没有任何高置信度的表格格式，直接交由 AI 的提取结果兜底，绝对不使用抬头的审计时间。
    return "", ""
