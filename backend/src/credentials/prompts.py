...
- 同一交接单可能有**多联**：第一联（存根联）的印章通常是**黑色**，第二联（套打联）的印章通常是**蓝色**。颜色是区分联次的关键依据。
...
4. **颜色识别与联次推断 (重要)**：
   - 仔细观察每枚印章（"业务受理章"圆圈及编码）的**主体颜色**（注意区分：黑色打印机墨 vs 蓝色套打墨水）。
   - color 取值：`black`（黑色）、`blue`（蓝色）、`other`（其他颜色或无法确定）。
   - 根据 color 推断 copy（联次）：
     - `black` → copy 为"第一联"
     - `blue`  → copy 为"第二联"
     - `other` → copy 为""
   - 注意：颜色判断以印章图案本身为准，不要被背景纸色干扰。
5. **单据标识提取 (用于判定是否为同一张单，极度重要)**：
   - 每枚印章所属表单的顶部通常印有**车号**、**线路**、**No.(单号)**三项标识。
   - 请为每枚印章提取它所在表单的：
     - `vehicle_no` (车号)：如"66-B51KD8"
     - `route` (线路)：如"A1001"
     - `form_no` (单号)：No. 后的编号，如"0833892"
   - 这些标识是判断多枚印章是否属于同一张交接单的依据。
   - **每一联都必须独立读取表头**：即使是粉色/套打的第二联（蓝色印章），其表头的车号/线路/No. 同样清晰印在纸张顶部，**绝不能因为颜色浅或背景色就跳过表头读取**。务必为每一联都完整填写 vehicle_no/route/form_no，禁止留空。

## 业务规则 (Critical - 编码一致性)：
- **同一张交接单的判定依据是 No.（单号 form_no）一致**：只要 form_no 相同（无论车号/线路是否完全一致），就视为同一张交接单。
- 同一张交接单的每一枚印章，其防伪编码在物理上**应当是一致的**（即同一张单的码理应相同）。
- 若你发现多枚印章的 form_no 相同，但读出的 code 不同，**很可能是 OCR 识别误差**。此时请对每枚印章的编码逐字符重新核对，尽量给出**统一的**正确编码。
- 反之，若 form_no 不同，则属于不同张单，编码不同是正常的，不要强行统一。

## 输出要求：
- header (表头): 提取影像顶部的**文件类型名称**（如"早送尾箱交接单"）。
- seal_details (电子印章详情列表): 返回提取到的**每一枚印章**的详情，含编码 code、颜色 color、联次 copy、车号 vehicle_no、线路 route、单号 form_no。
- seal_codes (编码列表): 与 seal_details 中 code 一一对应的编码列表（系统会以此做备份）。
...
## 输出格式 (严格 JSON):
{
    "header": "",
    "seal_details": [
        {"code": "编码1", "color": "black", "copy": "第一联", "vehicle_no": "66-B51KD8", "route": "A1001", "form_no": "0833892"},
        {"code": "编码2", "color": "blue",  "copy": "第二联", "vehicle_no": "66-B51KD8", "route": "A1001", "form_no": "0833892"}
    ],
    "seal_codes": ["编码1", "编码2"]
}
"""
...
# -----------------------
SETTLEMENT_APPLICATION_PROMPT = """
Role: 结算业务申请书信息提取与校对专家

Task: 从《结算业务申请书》影像中分别提取「左联（申请书主体）」和「右联（银行受理通知联）」的收款人户名、收款人账号、金额字段。

## 版式说明（极度重要）
- 本类单据被中间一条**垂直虚线**纵向分割为左右两部分：
  - **左联（申请书主体）**：虚线左侧的主表区域，包含申请人信息与收款人信息（户名、账号、开户行、金额、用途等），底部有申请人印章、银行受理章。
  - **右联（银行受理通知联）**：虚线右侧的窄联，通常印有"此联为银行受理通知书"或类似说明，同样有收款人户名、账号、金额等核心字段。
- 左联与右联是**同一份单据的两个联次**，对应字段的值在物理上**理应一致**，但必须各自独立识别，不得互相抄录。

## 提取要求
1. 版式判断 (is_settlement_application)：判断影像是否为"结算业务申请书"。是返回 true，否则返回 false。

2. 右联字段提取（**先提取右联**，虚线右侧区域）：
   - right_payee_name (右联收款人户名)：右联中"收款人"对应的户名/名称。
   - right_payee_account (右联收款人账号)：右联中收款人账号（银行账号，非手机号）。
   - right_amount (右联金额)：右联中的交易/结算金额，原样提取（保留¥、逗号、小数等原貌）。

3. 左联字段提取（再提取左联，虚线左侧区域）：
   - left_payee_name (左联收款人户名)：左联中"收款人"对应的户名/名称。
   - left_payee_account (左联收款人账号)：左联中收款人账号。
   - left_amount (左联金额)：左联中的交易/结算金额，原样提取。

## 字段含义约束（严禁错位）
- payee_account（收款人账号）只能填**银行账号**，禁止填手机号、客户号或金额。
- amount（金额）只能填金额，禁止填账号或用途。
- **严禁左右互相填充**：左联字段只能从虚线左侧读取，右联字段只能从虚线右侧读取，即使一侧为空也不得用另一侧内容顶替。
- 若某联某字段在影像中无内容（空白/未填写/无法辨认），返回空字符串 ""。

## 账号识别注意
- 账号逐位读取，可能含空格（如"6228 4802 4828 6521 773"），原样保留空格即可。
- 区分易混数字：1 vs 7、0 vs 6/9、3 vs 5、2 vs 7。

## 金额识别注意
- 金额原样提取，保留 ¥/￥、千分位逗号、小数点，如"¥4,000.00"。系统会自动做规范化比较。

## 输出规则
- 返回严格的 JSON 对象。
- 不要输出 markdown 标记及任何解释性文本。
- 未出现/无内容的字段返回空字符串 ""。

## JSON Schema 示例:
{
    "is_settlement_application": true,
    "right_payee_name": "",
    "right_payee_account": "",
    "right_amount": "",
    "left_payee_name": "",
...
PROMPT_MAPPING = {
    ...
    "settlement_application": SETTLEMENT_APPLICATION_PROMPT
}
