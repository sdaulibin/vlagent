...
2. 要素提取：
   ...
   - authorized_items (授权事项列表): **仅提取方框内打钩 (√) 的项目**。如果方框内是 **叉号 (X)**、**对角线交叉标记** 或者为空，则视为**未授权/不提取**。
     - **判定规则 (COT)**：对每个列表项，首先分析框内笔划物理形状。如果识别到“两条线段呈对角交叉”状，这属于 X，严禁提取。只有识别到明显的“一笔转折”钩形(√)才进行提取。请仔细辨别 √ 和 X。
