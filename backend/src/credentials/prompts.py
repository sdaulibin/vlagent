...
2. 特定要素提取：
   - 账号/银行卡�POWER_OF_ATTORNEY_PROMPT = """
Role: 银行授权委托书识别专家

Task: 从银行授权委托书影像中提取关键信息，逐项识别所有授权事项的勾选状态。
...
| 符号 | 特征 | 结果 |
|------|------|------|
| √ 对号 | **一笔**连续钩形，无交叉点 | checked: true |
| × 叉号 | **两笔**交叉，有中心交叉点 | checked: false |
...
**注意**：每个项目独立判断，不要根据其他项目的结果推测。文档中可能大部分是√，也可能大部分是×，或两者混合，都是正常的。
...
⚠️ **重要说明**：
- 授权委托书中，客户可能勾选少数项目，也可能勾选大部分甚至全部项目，这都是正常的
- **不要预设"应该有多少项被勾选"**，严格根据每个方框内的实际符号判断
- 每个项目**独立判断**，不受其他项目影响
...
### 四、其他业务（other）
- 这是手写或机打的其他授权事项填写区域
- 必须**完整、逐字**提取该区域的**所有文字内容**，一个字都不能漏
- 提取后再按分隔符拆分为独立项目

**拆分规则：**
- 顿号（、）：如 "签约电话对账、官网对账、自助回单业务" → 3项
- 逗号（，或,）：如 "签约电话对账，官网对账，自助回单业务" → 3项
- 空格：如 "签约电话对账 官网对账 自助回单业务" → 3项
- 混合分隔符：按任意分隔符拆分
...
**步骤1 - 定位方框**：找到该项目名称前面的方框 □ 位置

**步骤2 - 观察框内内容**：
- 如果框内完全空白 → checked: false
- 如果框内有墨迹/符号 → 进入步骤3
...
| 符号类型 | 几何特征 | 判定结果 |
|---------|---------|---------|
| √ 对号 | 一笔连续，从左上→右下→右上挑起，形成"钩形"，**无交叉点** | checked: **true** |
| × 叉号 | 两笔交叉，两条线段在**中心交叉**，形成"乘号" | checked: **false** |
| ☑ 填充 | 方框内被填满或有明显的对号标记 | checked: **true** |
| 空白 | 方框内无任何墨迹 | checked: **false** |

**关键区分点：有无交叉点**
- **有**明显的中心交叉点（两条线相交）→ 是 × → checked: false
- **无**交叉点，线条连续呈钩形 → 是 √ → checked: true
...
2. 每个项目用对象表示：
   ```json
   {
     "name": "项目名称",
     "checked": true/false,
     "bbox": [x1, y1, x2, y2]  // 方框的坐标（左上角和右下角），用于视觉验证
   }
   ```
3. 其他业务：按分隔符拆分为独立项目，以字符串数组形式输出
4. 返回严格的 JSON 对象，不要输出任何解释性文字

**注意：** bbox 坐标非常重要，用于后续的视觉验证。请尽量准确地标注方框位置。
...
# -----------------------
# 9. 符号识别专用 (Symbol Recognition for Checkmarks) - 思维链版
# -----------------------
SYMBOL_RECOGNITION_PROMPT = """
Role: 勾选符号识别专家

Task: **仅关注符号本身**，判断方框内的符号是 √（对号）还是 ×（叉号）。
...
### 步骤2：观察框内内容
- 方框内是否有墨迹？
- 如果完全空白 → 直接判断为"空白" → is_checkmark: false
- 如果有墨迹 → 进入步骤3
...
### 步骤4：描述符号形状
**根据步骤3的观察，描述符号的具体形状：**
- 如果有交叉点：描述为"两条线交叉，形成X形"
- 如果无交叉点：描述为"一笔钩形，从左上到右下再到右上"
- 如果不确定：描述为"模糊不清"或"无法确定"

### 步骤5：最终判断
**根据前面的分析，做出最终判断：**
- 如果有交叉点 → is_checkmark: false
- 如果无交叉点且是钩形 → is_checkmark: true
- 如果不确定 → is_checkmark: false
    ...
    "symbols": [
        {
            "item_name": "账户开户",
            "has_ink": true,
            "intersection_analysis": "两条线在中心交叉，形成明显的交叉点",
            "has_intersection": true,
            "symbol_description": "两条线交叉，形成X形",
            "is_checkmark": false
        },
        {
            "item_name": "企业网上银行注册",
            "has_ink": true,
            "intersection_analysis": "一条连续的线，没有交叉点",
            "has_intersection": false,
            "symbol_description": "一笔钩形，从左上到右下再到右上",
            "is_checkmark": true
        }
    ]
}
...
PROMPT_MAPPING = {
    ...
    "symbol_recognition": SYMBOL_RECOGNITION_PROMPT
}
