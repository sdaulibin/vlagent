...
- table_headers: 该事项下表格的所有列名（按从左到右顺序）。⚠️ 某些列可能很窄（如"币种"、"备注"仅两字宽），请逐列仔细检查不要遗漏，可通过数据行的列数验证表头数量。注意：如果是多级表头（例如“款项来源”下有“境内”和“境外”），请使用嵌套字典，如 {"款项来源": ["境内", "境外"]}
...
def _normalize_section_name(name: str) -> str:
    ...
    - 去除编号前缀（如 "3. "）
    - 去除多余空格（方便后续处理带空格的日期）
    - 去除日期周围的方括号（如 [2022年01月01日] → 2022年01月01日）
