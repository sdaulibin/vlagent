...
def read_summary_data(file_path, schema_name: str = "bank_summary"):
    ...
    prompt = f"""
    你是一个专业的银行单据信息提取专家。请从图片中提取银行流水的汇总信息。
    
    ⚠️ 【最重要警告 - 开户行识别】⚠️
    "开户行"字段必须从【红色印章】中提取，而不是从账户名称推断！
    
    ❌ 常见错误示例：
    - 看到账户名"青岛XX公司" → 错误地填写"青岛银行" ← 这是错误的！
    - 看到账户名"潍坊XX公司" → 错误地填写"潍坊银行" ← 这是错误的！
    账户名中的城市名称与银行名称无关！
    
    ✅ 正确做法：
    1. 找到图片右上角的【红色圆形或椭圆形印章】
    2. 印章边缘（外围）有银行名称，通常格式为"XX银行股份有限公司"
    3. 逐字阅读印章边缘的文字，提取完整银行名称
    
    【印章文字阅读步骤】
    1. 定位红色印章（通常在右上角）
    2. 印章边缘文字沿圆周排列，通常从左上开始顺时针阅读
    3. 印章中央可能有五角星，下方通常是章类型（如"电子回单专用章"）
    4. 请将边缘的完整银行名称填入"开户行"字段
    ...
    【数值字段格式要求】
    - "收入总笔数"和"支出总笔数"：只输出纯数字，如 "40"
    - "收入总金额"和"支出总金额"：只输出纯金额数字，如 "26449356.12"
    ...
    请只输出合法的JSON格式，不需要任何解释。
    如果某个字段在图片中找不到，请填写空字符串。
